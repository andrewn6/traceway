<script lang="ts">
	...
	const tabs = [
		{ href: '/settings', label: 'General', icon: 'general' },
		{ href: '/settings/providers', label: 'Providers', icon: 'providers' },
		{ href: '/settings/api-keys', label: 'API Keys', icon: 'api-keys' },
		{ href: '/settings/team', label: 'Team', icon: 'team' },
		{ href: '/settings/usage', label: 'Usage', icon: 'usage' },
...
<div class="app-shell-wide space-y-4 motion-rise-in">
	<nav class="flex items-center gap-1 border-b border-border/40 pb-2" aria-label="Settings sections">
		{#each tabs as tab}
			<a
				href={tab.href}
				class="query-chip text-[12px] inline-flex items-center gap-1.5 {isActive(tab.href) ? 'query-chip-active' : ''}"
			>
				{#if tab.icon === 'general'}
