<script lang="ts">
	import type { ArticleMeta } from './+page.ts';

	let { data } = $props();
	let articles: ArticleMeta[] = $derived(data.articles);
...
<svelte:head>
	<title>Blog - Traceway</title>
	<meta name="description" content="Articles about LLM observability, AI debugging, and building reliable AI applications." />
</svelte:head>
...
<nav class="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-b border-border/50">
	<div class="max-w-[1200px] mx-auto flex items-center justify-between px-6 py-3">
		...
		<div class="flex items-center gap-3">
			<a href="/" class="text-[14px] text-text-secondary hover:text-text transition-colors">Home</a>
			<a
				href="https://platform.traceway.ai/signup"
				class="text-[13px] font-semibold bg-accent text-white rounded-lg px-4 py-2 hover:bg-accent-dim transition-all"
			>
				Sign Up
			</a>
		</div>
...
<main class="pt-28 pb-20">
	<div class="max-w-[720px] mx-auto px-6">
		<h1 class="text-[clamp(2rem,5vw,3rem)] font-bold tracking-tight text-text">Blog</h1>
		<p class="text-text-secondary mt-3 text-[16px]">Thoughts on LLM observability, AI debugging, and building reliable AI applications.</p>

		<div class="mt-12 space-y-1">
			{#each articles as article}
				<a
					href="/blog/{article.slug}"
					class="block group -mx-4 px-4 py-6 rounded-xl hover:bg-bg-secondary transition-colors border-b border-border/50 last:border-0"
				>
					<div class="flex items-baseline justify-between gap-4 mb-2">
						<h2 class="text-[18px] font-semibold text-text group-hover:text-accent transition-colors leading-snug">{article.title}</h2>
						<time class="text-[13px] text-text-muted shrink-0">{formatDate(article.date)}</time>
					</div>
					<p class="text-[14px] text-text-secondary leading-relaxed">{article.description}</p>
				</a>
			{/each}
		</div>
	</div>
