...
<svelte:head>
	<title>{data.title} - Traceway Blog</title>
...
<article class="pt-28 pb-20">
	<div class="max-w-[680px] mx-auto px-6">
		<!-- Back link -->
		<a href="/blog" class="inline-flex items-center gap-1.5 text-[13px] text-text-muted hover:text-text transition-colors mb-8">
		...
		<!-- Header -->
		<header class="mb-10">
			<h1 class="text-[clamp(1.8rem,4.5vw,2.8rem)] font-bold tracking-tight text-text leading-[1.15]">{data.title}</h1>
			<p class="text-text-secondary mt-4 text-[16px] leading-relaxed">{data.description}</p>
			<div class="flex items-center gap-3 mt-5 text-[13px] text-text-muted">
